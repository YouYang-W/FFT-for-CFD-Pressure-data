...
%% 读取数据
addpath('G:\codeStorage\matlab\readFluentOut'); % 读取程序模块
T = readFluentOut('J:\Doctor\Re15000\report-def-0-rfile_10002.out');
T_copy = T;

p_ref   = 1e-6;                               % 参考声压
L       = length(T_copy.("flow-time"));       % 信号长度
Fs      = 1/mean(diff(T_copy.("flow-time"))); % 采样率
delta_f = Fs/L;                               % 频率分辨率
fprintf('频率分辨率：%.2f Hz\n' , delta_f);
fprintf('信号持续时间：%.2f s\n', L/Fs);  
d       = 0.01822;
...
%% FFT
Signal = T_copy.("down-wall-1");% (1000:end);
...
PSD_slope_line    = 1e-8 * (St_slope_range).^(-11/3); % 计算参考线的值
...
legend([h_data, h_slope], {'Re = 25000', '$St^{-11/3}$'}, ...
